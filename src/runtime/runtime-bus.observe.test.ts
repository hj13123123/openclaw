...
import { readLatestRuntimeLoopState, tick } from "./runtime-loop.js";
...
function task(taskId: string, status: TaskRecord["status"], overrides: Partial<TaskRecord> = {}): TaskRecord {
  return {
    taskId,
    status,
...
describe("runtime bus observe-only validation", () => {
  it("persists runtime events and returns the requested recent tail", () => withTempRoot((workspaceRoot) => {
  ...
  it("builds task state with last record winning for duplicate task ids", () => withTempRoot((workspaceRoot) => {
    appendTask(workspaceRoot, task("TASK-1", "queued", { updatedAt: "2026-05-20T00:00:00.000Z" }));
    appendTask(workspaceRoot, task("TASK-1", "completed", { updatedAt: "2026-05-20T00:01:00.000Z" }));
  ...
  it("writes dry-run policy action audit without moving source files", () => withTempRoot((workspaceRoot) => {
    ...
    appendTask(workspaceRoot, task("RETURN-A", "queued", {
      metadata: { sourcePath },
    ...
    expect(readJsonl(path.join(workspaceRoot, "runtime", "policy", "action-audit.jsonl"))).toHaveLength(1);
    expect(getRecentEvents(workspaceRoot, 2).map((event) => event.eventType)).toContain("policy_action_dry_run_completed");
  }));

  it("runs runtime loop tick in observe mode without writing dispatch requests", () => withTempRoot((workspaceRoot) => {
    ...
    expect(state.warnings).toContain("schedulerPolicy.maxDispatchesPerTick is non-zero, dispatch still suppressed by observe-only loop");
