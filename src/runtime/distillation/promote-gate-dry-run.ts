...
const ALLOWED_CANDIDATE_TYPES = new Set(["skill", "rule", "memory", "engineering_rule", "checklist"]);
...
function detectFrozen(workspaceRoot: string): boolean {
  ...
  return /runtime frozen|auto-progression.*frozen|auto-chain.*frozen|auto-evolution.*frozen/u.test(texts.join("\n"));
}
...
function sourceExists(workspaceRoot: string, source: unknown): PromoteGateSourceCheck {
  ...
  if (sourceType === "case") {
    ...
    return { sourceType, sourceId, pass: existsSync(filePath), path: relativePath(workspaceRoot, filePath), reason: null };
  }
  ...
  if (sourceType === "return") {
    ...
    const matched = existsSync(exact)
      ? exact
      : listJsonFiles(processedDir).find((filePath) => path.basename(filePath, ".json").startsWith(normalized));
    return {
      sourceType,
      sourceId,
      pass: Boolean(matched),
  ...
  return { sourceType, sourceId, pass: false, path: null, reason: `unsupported sourceType: ${sourceType}` };
}

function resolvePromoteTarget(candidateType: string): { promoteTarget: string; requiredGate: string; warnings: string[] } {
  if (candidateType === "memory") {
    return {
      promoteTarget: "D1 controlled apply candidate",
      requiredGate: "D1_CONTROLLED_APPLY_REQUIRED",
      warnings: ["memory candidates must be routed to D1 controlled apply; no direct MEMORY.md write"],
    };
  }
  if (candidateType === "engineering_rule") {
    return {
      ...
      warnings: ["engineering_rule candidates require separate explicit approval; no direct ENGINEERING_RULES.md write"],
    };
  }
...
function evaluateCandidate(params: {
  ...
  checks.push({ name: "candidateType_allowed", pass: ALLOWED_CANDIDATE_TYPES.has(candidateType), value: candidateType });
  checks.push({ name: "review_exists", pass: Boolean(review), reviewId: stringValue(review?.reviewId) });
  ...
  for (const gateName of ["duplicateCheck", "conflictCheck", "minimumOccurrence", "confidenceThreshold"]) {
  ...
  checks.push({ name: "confidence_minimum", pass: confidencePass, value: confidence, minimum: minConfidence });
  ...
  checks.push({ name: "occurrence_minimum", pass: occurrencePass, value: occurrenceCount, minimum: 2 });
  ...
  checks.push({ name: "sources_resolvable", pass: sourcePass, total: sourceResults.length, sources: sourceResults });
  ...
  if (review) {
    ...
    for (const item of ["evidenceSufficient", "noDuplicate", "noConflict", "notOverfit", "noPrivilegeExpansion"]) {
    ...
    checks.push({ name: "review_check_needsMoreEvidence_false", pass: needsMoreEvidencePass, value: checklist.needsMoreEvidence ?? null });
    if (!needsMoreEvidencePass) blockers.push("review_needs_more_evidence");

    const noTruthFileTouch = checklist.noTruthFileTouch === true;
    checks.push({ name: "review_check_noTruthFileTouch", pass: noTruthFileTouch, value: checklist.noTruthFileTouch ?? null });
    if (!noTruthFileTouch && candidateType !== "memory") blockers.push("truth_file_touch_not_allowed_for_type");

    const reviewApproved =
      ["approved", "accepted"].includes((stringValue(review.reviewStatus) ?? "").toLowerCase()) ||
      ["approved", "approve", "accepted"].includes((stringValue(review.verdict) ?? "").toLowerCase());
    checks.push({
  ...
  if (blockers.includes("waiting_for_human_review_approval") || blockers.includes("missing_review_candidate")) verdict = "WAITING_REVIEW";
  if (blockers.includes("review_needs_more_evidence") || blockers.includes("confidence_below_runtime_threshold")) verdict = "NEEDS_EVIDENCE";
  ...
  if (blockers.length === 0 && candidateType === "engineering_rule") verdict = "WAITING_SEPARATE_ENGINEERING_RULE_APPROVAL";
...
export function runPromoteGateDryRun(
  ...
  const outputFile = options.outputPath === undefined
    ? path.join(workspaceRoot, PROMOTE_GATE_REPORT_DIR_RELATIVE_PATH, `d9-promote-gate-dryrun-${generatedAt.replace(/[:.]/gu, "-")}.json`)
    : options.outputPath;
...
export function summarizePromoteGateDryRun(report: PromoteGateDryRunReport): Pick<
  PromoteGateDryRunReport,
  "status" | "mode" | "generatedAt" | "frozenActive" | "outputFile" | "stats" | "constraintsVerified"
> {
  return {
