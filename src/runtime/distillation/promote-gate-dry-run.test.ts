...
describe("promote gate dry-run", () => {
  ...
  it("marks a reviewed skill candidate ready without promoting it", () => {
    ...
    writeJson(
      path.join(root, "runtime", "main", "tmp", "distill-candidates", "cand-skill.json"),
      candidate(),
    );
    writeJson(
      path.join(root, "runtime", "main", "tmp", "review-candidates", "review-skill.json"),
      review(),
    );
  ...
  it("routes memory and engineering_rule candidates to explicit gates", () => {
    const root = workspace();
    writeJson(
      path.join(root, "runtime", "main", "tmp", "distill-candidates", "cand-memory.json"),
      candidate({
    ...
    writeJson(
      path.join(root, "runtime", "main", "tmp", "distill-candidates", "cand-rule.json"),
      candidate({
    ...
    writeJson(
      path.join(root, "runtime", "main", "tmp", "review-candidates", "review-memory.json"),
      review({
    ...
    writeJson(
      path.join(root, "runtime", "main", "tmp", "review-candidates", "review-rule.json"),
      review({
    ...
    expect(report.plans.find((plan) => plan.candidateId === "cand-memory")).toEqual(
      expect.objectContaining({
        verdict: "ROUTE_D1_CONTROLLED_APPLY",
        requiredGate: "D1_CONTROLLED_APPLY_REQUIRED",
      }),
    );
    expect(report.plans.find((plan) => plan.candidateId === "cand-rule")).toEqual(
      expect.objectContaining({
  ...
  it("writes only the dry-run report when an output path is provided and frozen blocks auto promote", () => {
    const root = workspace();
    writeFileSync(
      path.join(root, "SESSION_SUMMARY.md"),
      "D10 runtime frozen; auto-evolution frozen\n",
      "utf8",
    );
    writeJson(
      path.join(root, "runtime", "main", "tmp", "distill-candidates", "cand-skill.json"),
  ...
  it("reads the latest dry-run state without gateway filesystem logic", async () => {
    const root = workspace();
    writeJson(
      path.join(root, "runtime", "main", "tmp", "distill-candidates", "cand-skill.json"),
      candidate(),
    ...
    expect(state).toEqual(
      expect.objectContaining({
        available: true,
        reportPath: "runtime/main/tmp/d9-promote-gate-dryrun-2026-05-20T10-04-00-000Z.json",
        status: "PASS",
        mode: "dry-run",
        generatedAt: "2026-05-20T10:04:00.000Z",
  ...
  it("reports invalid latest dry-run state without throwing", async () => {
    const root = workspace();
    const reportPath = path.join(
      ...
      "main",
      "tmp",
      "d9-promote-gate-dryrun-2026-05-20T10-05-00-000Z.json",
    );
    mkdirSync(path.dirname(reportPath), { recursive: true });
    writeFileSync(reportPath, "{invalid", "utf8");
    ...
    expect(state).toEqual(
      expect.objectContaining({
        ...
        reportPath: "runtime/main/tmp/d9-promote-gate-dryrun-2026-05-20T10-05-00-000Z.json",
        error: expect.stringContaining("Promote gate state read failed"),
      }),
    );
  });
});
