...
import { buildReturnConsumptionArtifacts } from "./return-consumer-artifacts.js";
import {
...
export type ReturnConsumerLog = { info: (msg: string) => void; warn: (msg: string) => void };

export type ProcessReturnInboxResult = {
...
export function processReturnInbox(
  workspaceRoot: string,
  log: ReturnConsumerLog,
): ProcessReturnInboxResult[] {
  ...
  const results: ProcessReturnInboxResult[] = [];
  try {
...
export function ensureReturnConsumerDirs(workspaceRoot: string): void {
  fs.mkdirSync(path.join(workspaceRoot, "system", "returns", "inbox"), { recursive: true });
  fs.mkdirSync(path.join(workspaceRoot, "system", "returns", "processed"), {
    recursive: true,
  });
  fs.mkdirSync(path.join(workspaceRoot, "runtime", "main", "tmp"), { recursive: true });
  fs.mkdirSync(path.join(workspaceRoot, "runtime", "notifications", "inbox"), {
    recursive: true,
  });
}

export function writeReturnConsumerErrorReport(
  workspaceRoot: string,
  err: unknown,
  sourceFile?: string,
): void {
  writeErrorReport(workspaceRoot, err, sourceFile);
}

function processReturnFile(workspaceRoot: string, fileName: string): ProcessReturnInboxResult {
  ...
  writeJson(path.join(processedDir, artifacts.receiptFileName), artifacts.receipt);
...
function writeBatchNotice(workspaceRoot: string, results: ProcessReturnInboxResult[]): void {
