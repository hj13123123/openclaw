...
describe("mirror observe", () => {
  ...
  it("generates observe-only findings from D7/D8/D9 runtime state", () => {
    ...
    expect(report.observations.map((item) => item.source)).toEqual(["hud", "scheduler", "kb", "promote_gate"]);
    ...
    expect(report.findings.find((finding) => finding.source === "promote_gate")).toEqual(expect.objectContaining({
      severity: "attention",
  ...
  it("reports missing runtime inputs as warnings without applying changes", () => {
    ...
    expect(report.findings.every((finding) => finding.suggestedAction.includes("do not auto-apply changes from mirror observe"))).toBe(true);
