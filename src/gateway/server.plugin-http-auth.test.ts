...
async function expectHealthzProbeBypassesPlugin(params: {
...
describe("gateway plugin HTTP auth boundary", () => {
  ...
  test("keeps probe routes ahead of plugins for runtime health gates", async () => {
    ...
    await withGatewayServer({
      ...
      run: async (server) => {
        await expectHealthzProbeBypassesPlugin({ server, handlePluginRequest });
      },
    });
  });
  ...
  test("root-mounted control ui keeps probe paths ahead of plugins", async () => {
    const handlePluginRequest = createHealthzPluginHandler();
