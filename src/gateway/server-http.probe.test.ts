...
describe("gateway probe endpoints", () => {
  ...
  it("routes return diagnostics through the HTTP fast path", async () => {
    await withGatewayServer({
      prefix: "return-diagnosis-fast-path",
      resolvedAuth: AUTH_NONE,
      run: async (server) => {
        const req = createRequest({
          path: "/api/returns/diagnosis",
        });
        const { res, getBody } = createResponse();
        await dispatchRequest(server, req, res);
  ...
  it("routes return repair dry-runs through the HTTP fast path", async () => {
    await withGatewayServer({
      prefix: "return-repair-dry-run-fast-path",
      resolvedAuth: AUTH_NONE,
      run: async (server) => {
        const req = createRequest({
          path: "/api/returns/repair-dry-run",
        });
        const { res, getBody } = createResponse();
        await dispatchRequest(server, req, res);
  ...
  it("routes return reconciliation gates through the HTTP fast path", async () => {
    await withGatewayServer({
      ...
      run: async (server) => {
        const req = createRequest({
          path: "/api/returns/reconciliation-gate",
        });
        const { res, getBody } = createResponse();
        await dispatchRequest(server, req, res);
  ...
  it("routes return reconciliation apply plans through the HTTP fast path", async () => {
    await withGatewayServer({
      prefix: "return-reconciliation-apply-plan-fast-path",
      resolvedAuth: AUTH_NONE,
      run: async (server) => {
        const req = createRequest({
          path: "/api/returns/reconciliation-apply-plan",
        });
        const { res, getBody } = createResponse();
        await dispatchRequest(server, req, res);
  ...
  it("routes position config audits through the HTTP fast path", async () => {
    await withGatewayServer({
      prefix: "position-config-audit-fast-path",
      resolvedAuth: AUTH_NONE,
      run: async (server) => {
        const req = createRequest({
          path: "/api/positions/audit",
        });
        const { res, getBody } = createResponse();
        await dispatchRequest(server, req, res);
  ...
  it("routes position config cleanup plans through the HTTP fast path", async () => {
    await withGatewayServer({
      prefix: "position-config-cleanup-plan-fast-path",
      resolvedAuth: AUTH_NONE,
      run: async (server) => {
        const req = createRequest({
          path: "/api/positions/cleanup-plan",
        });
        const { res, getBody } = createResponse();
        await dispatchRequest(server, req, res);
  ...
  it("routes position config cleanup gates through the HTTP fast path", async () => {
    await withGatewayServer({
      prefix: "position-config-cleanup-gate-fast-path",
      resolvedAuth: AUTH_NONE,
      run: async (server) => {
        const req = createRequest({
          path: "/api/positions/cleanup-gate",
        });
        const { res, getBody } = createResponse();
        await dispatchRequest(server, req, res);
